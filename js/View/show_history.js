pattern_vis.View.prototype.show_history = function(){
  ...
  for( event_id in this.event_history ){
    ...
    var from_pos_x = from_view.pos_x * 1.0
                    + MARGIN.graph.left
                    + from_d3_vis_val.attr( "x" ) * 1.0
                    + from_d3_vis_val.attr( "width" ) / 2.0;
    var from_pos_y = from_view.pos_y * 1.0
                    + MARGIN.graph.top
                    + from_d3_vis_val.attr( "y" ) * 1.0
                    + from_d3_vis_val.attr( "height" ) / 2.0;

    var to_d3_vis_val = this.event_history[ event_id ].to_d3_vis_val;
    var to_pos_x = this.pos_x * 1.0
                    + MARGIN.graph.left
                    + to_d3_vis_val.attr( "x" ) * 1.0
                    + to_d3_vis_val.attr( "width" ) / 2.0;
    var to_pos_y = this.pos_y * 1.0
                    + MARGIN.graph.top
                    + to_d3_vis_val.attr( "y" ) * 1.0
                    + to_d3_vis_val.attr( "height" ) / 2.0;
